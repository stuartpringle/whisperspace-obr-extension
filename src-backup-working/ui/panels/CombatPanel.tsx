...
import { setTokenInitiative } from "../../obr/initiative";
...
const COMBAT_LOG_CHANNEL = "whisperspace.obr.sheet/combat-log";
...
useEffect(() => {
  ...
  async function rollInitiative() {
    const ref = props.sheet.attributes.ref ?? 0;
    const notation = `1d12 # Initiative +${ref}`; // Dice+ wants modifier after label
    const total = await rollWithDicePlusTotal({ diceNotation: notation });
    await setTokenInitiative(props.tokenId, total);
  }
  ...
  function skillModifier(skillId: string): number {
    ...
    if (learnedInfo && learnedInfo.focus === learningFocus) return 0;
...
function updateWeapon(i: number, patch: Partial<CharacterSheetV1["weapons"][number]>) {
  ...
  async function rollWeaponAttack(i: number, w: CharacterSheetV1["weapons"][number]) {
    ...
    const label = `Attack: ${w.name} (DC ${w.useDC})`;
    const diceNotation = buildWhisperspaceSkillNotation({ netDice, modifier: mod, label });

// Try to capture the numeric total (for hit/miss messaging). If Dice+ doesn't return a roll-result,
// we still fire the roll (fallback) and just skip the derived message.
let total: number | null = null;
try {
  total = await rollWithDicePlusTotal({ diceNotation, showResults: true, rollTarget: "everyone", timeoutMs: 4000 });
} catch (e) {
  // Fallback: still roll visually via Dice+ (fire-and-forget)
  await rollWithDicePlus({ diceNotation, showResults: true, rollTarget: "everyone" });
  console.warn("Dice+ total not received for weapon attack:", e);
}

if (typeof total === "number") {
  const dc = Number.isFinite(w.useDC as any) ? Number(w.useDC) : 0;
  const hit = total >= dc;
  const margin = total - dc;

  const baseDmg = Number.isFinite(w.damage as any) ? Number(w.damage) : 0;
  ...
  const totalDmg = baseDmg + critExtra;

  const msg = hit
    ? critExtra > 0
      ? `Extreme Success — CRIT! ${w.name} hits (rolled ${total} vs DC ${dc}; +${critExtra} dmg, +1 Stress). Deals ${totalDmg} damage.`
      : `Success! ${w.name} hits (rolled ${total} vs DC ${dc}). Deals ${baseDmg} damage.`
    : `Miss. ${w.name} rolled ${total} vs DC ${dc}.`;
  ...
  return (
    <Stack spacing={2}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
        ...
        <Button variant="contained" onClick={() => void rollInitiative()}>
          Roll Initiative
        </Button>
